...
| HDR/SDR 跨屏与全屏/DPI | 每次切换前后的显示器和输出日志 | 全屏/缩放通过；跨屏未验证（单显示器）；DPI 不感知已修，4K 原尺寸表面待复测 |
| 性能与关闭/恢复 | 丢帧、Present、内存、错误与释放顺序日志 | 2026-09-12 RTX 3070 短段通过（0 异常、退出码 0）；长时间与 4K 原尺寸负载待复测 |
