...
切换色彩输出时先停止呈现、在 UI 线程解除 SwapChainPanel，再经命令线程设置 mpv 色彩、经渲染线程替换 SwapChain/EGL surface，最后重新绑定。ANGLE 的 EGL_KHR_no_config_context 允许保留同一 EGL/mpv render context 和暂停视频帧。HDR 输出协商失败时，mpv 色彩与 SwapChain 一起回退到 SDR，不能把 PQ 像素交给 SDR 色彩空间。
