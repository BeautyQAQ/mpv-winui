...
**EOF 后信息面板清空（已修）。** mpv 在 EndFile 前把 `video-params`、`hwdec-current` 等逐项置 null。后端现在忽略媒体信息属性的 null 更新，保留最后信息直到下一次 StartFile。

## RTX 3070 第三轮复测（2026-09-12）

用户使用 `8eb84fa` 的测试包复测三项，日志分析见 [第三轮分析报告](../../artifacts/rtx3070-analysis/114448-963fd26d/分析报告.md)，验收记录见 [`evidence/P0-10-02-rtx3070-dpi-4k-retest.md`](evidence/P0-10-02-rtx3070-dpi-4k-retest.md)。DPI 修复生效：启动窗口 1658×1084 物理像素，全屏 `Window size` 3840×2160、`Video scale` 1.000；这是首次真正的 4K 表面输出。LG 4K60 HDR TS 全屏 PQ 输出 24 个 300 帧区间平均 5.005 秒（59.94 fps），稳态最大 Render 10.79 ms；呈现丢帧只在两次播放中开关 Windows HDR 时各增加 6 至 7 帧，之后两分钟不变。6 次 EOF 后信息面板均保留；28 次窗口尺寸切换无异常；整场 0 条 libmpv 错误/警告、0 次渲染等待、退出码 0。用户观察文字明显清晰、不掉帧、GPU 占用 25% 至 40%。
...
| 性能与关闭/恢复 | 丢帧、Present、内存、错误与释放顺序日志 | 2026-09-12 第三轮：4K60 HDR TS 全屏 59.94 fps、稳态 Render ≤ 10.79 ms、丢帧仅在 HDR 切换时增加；0 异常、退出码 0；长时间稳定性按用户决定不验证 |
