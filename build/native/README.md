...
3. 运行 `build-mpv.ps1`，通过 `-AngleSourcePath` 指向上一步的锁定 ANGLE 源码，通过 `-NasmPath` 指向锁定 NASM 2.16.03；脚本验证源码 commit 与 NASM 哈希。脚本自行检出所有锁定源码和 WrapDB 包，应用仓库中的五个补丁，以 Clang/LLD、`/MT` 和 LGPL 兼容选项构建单一 `libmpv-2.dll`。dav1d 采用静态库，启用 8/10/12 bit 软件 AV1 解码与 x64 SIMD。已有构建目录可传 `-Incremental`，在清除探测缓存后重配置并复用未受影响的对象文件；添加新子项目时须省略该开关，完整重配置以发现新选项。
